...
async function firstConfiguration(con){
	try{
		...
		await con.query('CREATE TABLE driver (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), telegram_id VARCHAR(255), phone VARCHAR(255), status BOOLEAN DEFAULT TRUE, document VARCHAR(255), balance INT DEFAULT 0, acceptance BOOLEAN DEFAULT FALSE)');
